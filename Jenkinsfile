pipeline{
    ...
    stages{
        ...
        stage("Trivy Scan") {
            steps {
                script {
		   bat ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image pavithragrblue/complete-e2e-app-success --no-progress --scanners vuln  --exit-code 0 --severity HIGH,CRITICAL --format table')
                }
	    }
